...
const SubgroupSettingsModal = ({ subgroup, projectId, isOwner, onClose, onUpdate, onDelete }) => {
    ...
    const availableMembers = projectMembers.filter(pm =>
        !members.some(m => m.userId === pm.userId) && pm.role !== 'VIEWER'
    );
    ...
    const transliterateTitle = (title) => { const m = {"а":"a","б":"b","в":"v","г":"g","д":"d","е":"e","ё":"yo","ж":"zh","з":"z","и":"i","й":"y","к":"k","л":"l","м":"m","н":"n","о":"o","п":"p","р":"p","с":"s","т":"t","у":"u","ф":"f","х":"h","ц":"ts","ч":"ch","ш":"sh","щ":"sch","ъ":"","ы":"y","ь":"","э":"e","ю":"yu","я":"ya","А":"A","Б":"B","В":"V","Г":"G","Д":"D","Е":"E","Ё":"Yo","Ж":"Zh","З":"Z","И":"I","Й":"Y","К":"K","Л":"L","М":"M","Н":"N","О":"O","П":"P","Р":"R","С":"S","Т":"T","У":"U","Ф":"F","Х":"H","Ц":"Ts","Ч":"Ch","Ш":"Sh","Щ":"Sch","Ъ":"","Ы":"Y","Ь":"","Э":"E","Ю":"Yu","Я":"Ya"}; return title.split("").map(c=>m[c]||c).join("").toLowerCase().replace(/[^a-z0-9]+/g,"_").replace(/^_+|_+`$/g,"").slice(0,50); }; const handleExportSubgroup = async () => {
        ...
        try {
            ...
            a.href = url;
            a.download = `group_${subgroup.id}_${transliterateTitle(subgroup.name)}.zip`;
            ...
            console.error(err);
            alert('Ошибка экспорта группы: ' + err.message);
        } finally {
                ...
                <div className="flex-row" style={{ marginTop: '20px', justifyContent: 'space-between' }}>
                    {isOwner && (
                        <>
                            ...
                            <button className="btn btn--secondary" onClick={handleExportSubgroup} disabled={isExporting}>
                                <i className="fas fa-download"></i> {isExporting ? 'Экспорт...' : 'Экспорт группы'}
                            </button>
                        </>
                    )}
